...
const COLORS = { bgTop:"#f5f3ff", bgBottom:"#f8fafc", card:"#ffffff", text:"#1f2330", muted:"#667085", border:"#ebe7f5", primary:"#7c3aed", primary2:"#a855f7", primary3:"#ec4899", primarySoft:"#f3e8ff", green:"#15803d", greenSoft:"#dcfce7", red:"#dc2626", redSoft:"#fee2e2", blue:"#2563eb", blueSoft:"#dbeafe", amber:"#d97706", amberSoft:"#fef3c7", slateSoft:"#f8fafc" };

const pageStyle = { minHeight:"100vh", background:`linear-gradient(180deg, ${COLORS.bgTop} 0%, ${COLORS.bgBottom} 100%)`, padding:20, fontFamily:"Inter, Arial, sans-serif", color:COLORS.text };
const cardStyle = (extra={}) => ({ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:24, padding:18, boxShadow:"0 10px 30px rgba(71,56,135,0.06)", ...extra });
const inputStyle = { padding:14, width:"100%", boxSizing:"border-box", borderRadius:14, border:`1px solid ${COLORS.border}`, background:"#fff", color:COLORS.text, outline:"none" };
const stylesBtn = {
  ...
  activePill:{ background:COLORS.primarySoft, color:COLORS.primary, border:"1px solid #ddd6fe" },
};
const buttonStyle = (kind="primary") => ({ padding:"12px 16px", borderRadius:14, fontWeight:700, cursor:"pointer", transition:"all 0.2s ease", ...stylesBtn[kind] });
const badgeStyle = (bg, color) => ({ display:"inline-flex", alignItems:"center", gap:6, padding:"7px 11px", borderRadius:999, background:bg, color, fontWeight:700, fontSize:13, whiteSpace:"nowrap" });
const voteBadgeStyle = (vote) => vote === "love" ? badgeStyle(COLORS.primarySoft, COLORS.primary) : vote === "yes" ? badgeStyle(COLORS.greenSoft, COLORS.green) : badgeStyle(COLORS.redSoft, COLORS.red);
const isPositiveVote = (vote) => vote === "yes" || vote === "love";

function scoreCandidate(candidate, likedMeta){
  if(!likedMeta.length) return 0;
  let score = 0;
  likedMeta.forEach((meta)=>{
    meta.styles.forEach((s)=>{ if(candidate.styles.includes(s)) score += meta.weight * 3; });
    meta.tags.forEach((t)=>{ if(candidate.tags.includes(t)) score += meta.weight * 2; });
...
function similarityScore(a,b){
  let score = 0;
  a.styles.forEach((s)=>{ if(b.styles.includes(s)) score += 3; });
  a.tags.forEach((t)=>{ if(b.tags.includes(t)) score += 2; });
...
export default function App(){
  ...
  useEffect(()=>{
    if(profile?.id){ loadVotes(profile.id); loadPartnerAndMatches(profile); }
    else { setVotes({}); setPartner(null); setPartnerVotes({}); }
  },[profile]);

  const namesMap = useMemo(()=> Object.fromEntries(NAMES_DATABASE.map((n)=>[n.name,n])),[]);
  const namePool = useMemo(()=> NAMES_DATABASE.map((n)=>n.name),[]);
  const allStyles = useMemo(()=> [...new Set(NAMES_DATABASE.flatMap((n)=>n.styles))].sort(),[]);
  const allOrigins = useMemo(()=> [...new Set(NAMES_DATABASE.map((n)=>n.origin))].sort(),[]);
  const allVibes = useMemo(()=> [...new Set(NAMES_DATABASE.map((n)=>n.vibe))].sort(),[]);

  const favoriteNames = useMemo(()=> namePool.filter((n)=> votes[n] === "yes" || votes[n] === "love"), [votes,namePool]);
  const matchedNames = useMemo(()=> namePool.filter((n)=> isPositiveVote(votes[n]) && isPositiveVote(partnerVotes[n])), [votes,partnerVotes,namePool]);
  const filteredNamePool = useMemo(()=> deckFilter === "favorites" ? favoriteNames : deckFilter === "matches" ? matchedNames : namePool, [deckFilter,favoriteNames,matchedNames,namePool]);
  ...
  const summary = useMemo(()=> {
    ...
    return { no:v.filter((x)=>x==="no").length, yes:v.filter((x)=>x==="yes").length, love:v.filter((x)=>x==="love").length };
  }, [votes]);
  const recentVotes = useMemo(()=> Object.entries(votes).map(([babyName,vote])=>({babyName,vote})).reverse().slice(0,8), [votes]);
  const likedMeta = useMemo(()=> namePool.filter((n)=> votes[n] === "yes" || votes[n] === "love").map((n)=> ({...namesMap[n], weight: votes[n] === "love" ? 2 : 1})), [votes,namePool,namesMap]);
  ...
  useEffect(()=> { refreshSmartSuggestions(); }, [votes, exploreStyle, exploreOrigin, exploreVibe, exploreInitial]);
  ...
  function refreshSmartSuggestions(){
    ...
    const sorted = candidates.map((item)=> ({...item, score: scoreCandidate(item, likedMeta)})).sort((a,b)=> b.score - a.score || a.name.localeCompare(b.name)).slice(0,10);
    setSmartSuggestions(sorted);
  }

  function saveProfileLocally(profileData){ localStorage.setItem(STORAGE_KEY, JSON.stringify(profileData)); }
  function clearSavedProfile(){ localStorage.removeItem(STORAGE_KEY); }
  function generateCoupleCode(){ return Math.random().toString(36).substring(2,8).toUpperCase(); }
  ...
  async function loadVotes(profileId){
    ...
    try {
      ...
      if(error){ setMessage("Errore caricamento voti: " + error.message); return; }
      const mapped = {};
      (data || []).forEach((row)=> { mapped[row.baby_name] = row.vote; });
      setVotes(mapped);
    } catch(err){ setMessage("Errore inatteso: " + err.message); }
  ...
  async function loadPartnerAndMatches(currentProfile){
    ...
    try {
      ...
      if(partnerError){ setMessage("Errore caricamento partner: " + partnerError.message); return; }
      const foundPartner = partnerRows && partnerRows.length > 0 ? partnerRows[0] : null;
      setPartner(foundPartner);
      if(!foundPartner){ setPartnerVotes({}); return; }
      const { data: votesRows, error: votesError } = await supabase.from("votes").select("baby_name, vote").eq("profile_id", foundPartner.id);
      if(votesError){ setMessage("Errore caricamento voti partner: " + votesError.message); return; }
      const mapped = {};
      (votesRows || []).forEach((row)=> { mapped[row.baby_name] = row.vote; });
  ...
  async function createNewCouple(){
    if(!name.trim()){ setMessage("Inserisci il tuo nome"); return; }
    ...
    try {
      ...
      if(error){ setMessage("Errore Supabase: " + error.message); return; }
      setProfile(data); saveProfileLocally(data); setVotes({}); setPartner(null); setPartnerVotes({}); setMessage("Profilo creato! Il tuo codice coppia è: " + coupleCode); setJoinCode(""); setName("");
    } catch(err){ setMessage("Errore inatteso: " + err.message); }
  ...
  async function joinExistingCouple(){
    if(!name.trim()){ setMessage("Inserisci il tuo nome"); return; }
    if(!joinCode.trim()){ setMessage("Inserisci un codice coppia"); return; }
    ...
    try {
      ...
      if(checkError){ setMessage("Errore controllo codice: " + checkError.message); return; }
      if(!existingProfiles || existingProfiles.length === 0){ setMessage("Codice coppia non trovato"); return; }
      const { data, error } = await supabase.from("profiles").insert({ name: name.trim(), couple_code: normalizedCode }).select().single();
      if(error){ setMessage("Errore Supabase: " + error.message); return; }
      setProfile(data); saveProfileLocally(data); setVotes({}); setPartner(null); setPartnerVotes({}); setMessage("Profilo collegato correttamente alla coppia " + normalizedCode); setName(""); setJoinCode("");
    } catch(err){ setMessage("Errore inatteso: " + err.message); }
    finally { setLoading(false); }
  }
  ...
  async function handleVote(voteType){
    ...
    try {
      ...
      if(error){ setMessage("Errore salvataggio voto: " + error.message); return; }
      setVotes((prev)=> ({ ...prev, [currentName]: voteType }));
    } catch(err){ setMessage("Errore inatteso: " + err.message); }
    finally { setVoteSaving(false); }
  }

  async function refreshMatches(){ if(!profile) return; await loadPartnerAndMatches(profile); }
  ...
  async function resetMyVotes(){
    ...
    if(!confirmed) return;
    setMessage(""); setVoteSaving(true);
    try {
      const { error } = await supabase.from("votes").delete().eq("profile_id", profile.id);
      if(error){ setMessage("Errore reset voti: " + error.message); return; }
      setVotes({}); setDeckFilter("all"); setMessage("Tutti i tuoi voti sono stati azzerati");
    } catch(err){ setMessage("Errore inatteso: " + err.message); }
    finally { setVoteSaving(false); }
  }

  function logoutProfile(){ clearSavedProfile(); setProfile(null); setVotes({}); setPartner(null); setPartnerVotes({}); setName(""); setJoinCode(""); setMessage("Profilo scollegato da questo dispositivo"); }

  return (
    <div style={pageStyle}>
      <style>{`*{box-sizing:border-box}.app-shell{max-width:1200px;margin:0 auto}.stats-grid{display:grid;gap:14px;grid-template-columns:repeat(auto-fit,minmax(220px,1fr))}.main-grid{display:grid;gap:20px;grid-template-columns:minmax(320px,1.3fr) minmax(300px,1fr);align-items:start}.stack-grid{display:grid;gap:20px}.two-col{display:grid;gap:20px;grid-template-columns:repeat(auto-fit,minmax(280px,1fr))}.auth-grid{display:grid;gap:18px;grid-template-columns:repeat(auto-fit,minmax(320px,1fr))}.deck-actions{display:grid;gap:12px;grid-template-columns:repeat(3,minmax(0,1fr))}.chip-wrap{display:flex;gap:8px;flex-wrap:wrap}.hover-lift{transition:transform .2s ease,box-shadow .2s ease}.hover-lift:hover{transform:translateY(-2px);box-shadow:0 14px 34px rgba(71,56,135,.10)}.message-bar{position:sticky;bottom:14px;z-index:20}@media (max-width:980px){.main-grid{grid-template-columns:1fr}}@media (max-width:640px){.deck-actions{grid-template-columns:1fr}}`}</style>
      {checkingSession ? (
        ...
        <div className="app-shell" style={{maxWidth:780}}>
          <div className="hover-lift" style={cardStyle({padding:28,marginBottom:20,background:`linear-gradient(135deg, ${COLORS.primarySoft} 0%, #ffffff 100%)`})}>
            <div style={badgeStyle(COLORS.primarySoft, COLORS.primary)}>✨ V10 Smart Catalog Upgrade</div>
            <h1 style={{fontSize:38,marginBottom:10}}>Il Nome Perfetto</h1>
            <p style={{color:COLORS.muted,fontSize:16,lineHeight:1.6,marginBottom:0}}>500 nomi curati con significato, origine, categorie e vibe. Nessun costo AI, ma un catalogo molto più intelligente.</p>
          </div>
          ...
          <div className="hover-lift" style={cardStyle({padding:24,marginBottom:20,background:`linear-gradient(135deg, #ffffff 0%, ${COLORS.primarySoft} 100%)`})}>
            <div style={{display:"flex",justifyContent:"space-between",gap:16,flexWrap:"wrap",alignItems:"center"}}>
              <div>
                ...
                <p style={{color:COLORS.muted,marginBottom:0}}>Ciao <strong>{profile.name}</strong> — codice coppia <strong>{profile.couple_code}</strong> — catalogo: <strong>{NAMES_DATABASE.length} nomi</strong></p>
              </div>
          ...
          <div className="main-grid" style={{marginTop:20}}>
            <div className="stack-grid">
              <div className="hover-lift" style={cardStyle({padding:22,background:"linear-gradient(180deg, #ffffff 0%, #fcfcff 100%)"})}>
                <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:14}}>
                  <div>
                    ...
                    <p style={{margin:"6px 0 0 0",color:COLORS.muted,fontSize:14}}>Con meaning, origin, vibe e categorie visibili.</p>
                  </div>
                ...
                {votesLoading ? <p style={{color:COLORS.muted}}>Caricamento voti...</p> : currentName && currentMeta ? (
                  <div style={{position:"relative",borderRadius:30,padding:24,background:`linear-gradient(135deg, ${COLORS.primary} 0%, ${COLORS.primary2} 56%, ${COLORS.primary3} 100%)`,color:"white",minHeight:390,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:"0 24px 50px rgba(124,58,237,0.24)",overflow:"hidden"}}>
                    ...
                    <div style={{position:"relative"}}>
                      ...
                      <div className="chip-wrap" style={{marginBottom:8}}>
                        {[...currentMeta.styles.slice(0,3), currentMeta.vibe].map((label) => <span key={label} style={badgeStyle("rgba(255,255,255,0.18)","#fff")}>{label}</span>)}
                      </div>
                    </div>
              ...
              <div className="two-col">
                <div className="hover-lift" style={cardStyle()}>
                  ...
                  {smartSuggestions.length === 0 ? <p style={{color:COLORS.muted}}>Inizia a votare per ricevere suggerimenti personalizzati.</p> : <div style={{display:"grid",gap:10,marginTop:12}}>{smartSuggestions.slice(0,5).map((item)=><div key={item.name} style={{padding:12,borderRadius:16,background:COLORS.slateSoft,border:`1px solid ${COLORS.border}`}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center"}}><strong>{item.name}</strong><span style={badgeStyle(COLORS.primarySoft,COLORS.primary)}>{item.vibe}</span></div><div style={{fontSize:13,color:COLORS.muted,marginTop:6}}>{item.meaning}</div></div>)}</div>}
                </div>
                <div className="hover-lift" style={cardStyle()}>
                  ...
                  {!currentMeta ? <p style={{color:COLORS.muted}}>Seleziona o raggiungi un nome nel deck.</p> : similarToCurrent.length === 0 ? <p style={{color:COLORS.muted}}>Nessun nome simile disponibile.</p> : <div className="chip-wrap" style={{marginTop:12}}>{similarToCurrent.map((item)=><span key={item.name} style={badgeStyle(COLORS.primarySoft,COLORS.primary)}>{item.name}</span>)}</div>}
                </div>
              </div>
              ...
              <div className="two-col">
                <div className="hover-lift" style={cardStyle()}>
                  ...
                  {favoriteNames.length === 0 ? <p style={{color:COLORS.muted}}>Ancora nessun preferito.</p> : <div className="chip-wrap" style={{marginTop:12}}>{favoriteNames.slice(0,18).map((name)=><span key={name} style={badgeStyle(votes[name] === "love" ? COLORS.primarySoft : COLORS.greenSoft, votes[name] === "love" ? COLORS.primary : COLORS.green)}>{name}</span>)}</div>}
                </div>
                <div className="hover-lift" style={cardStyle()}>
                  ...
                  {recentVotes.length === 0 ? <p style={{color:COLORS.muted}}>I tuoi ultimi voti appariranno qui.</p> : <div style={{display:"grid",gap:10,marginTop:12}}>{recentVotes.map((item)=><div key={item.babyName} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:12,borderRadius:16,background:COLORS.slateSoft,border:`1px solid ${COLORS.border}`}}><span style={{fontWeight:600}}>{item.babyName}</span><span style={voteBadgeStyle(item.vote)}>{item.vote}</span></div>)}</div>}
                </div>
              </div>
            </div>
            ...
            <div className="stack-grid">
              <div className="hover-lift" style={cardStyle({background: matchedNames.length ? "linear-gradient(180deg, #f0fdf4 0%, #ffffff 100%)" : "#ffffff"})}>
                ...
                {matchLoading ? <p style={{color:COLORS.muted,marginTop:12}}>Caricamento match...</p> : !partner ? <div style={{marginTop:12}}><p style={{marginBottom:8}}><strong>Nessun partner collegato ancora.</strong></p><p style={{color:COLORS.muted,marginTop:0}}>Condividi il tuo codice coppia: <strong>{profile.couple_code}</strong></p></div> : matchedNames.length === 0 ? <p style={{color:COLORS.muted,marginTop:12}}>Per ora nessun match positivo.</p> : <><div style={{marginTop:12,marginBottom:14,padding:16,borderRadius:18,background:"rgba(34,197,94,0.08)",border:"1px solid #bbf7d0"}}><p style={{margin:0,color:COLORS.muted,fontSize:14}}>Compatibilità percepita</p><h3 style={{marginTop:8,marginBottom:0,fontSize:34}}>{Math.min(100, 50 + matchedNames.length * 10)}%</h3></div><div className="chip-wrap">{matchedNames.slice(0,18).map((n)=><span key={n} style={badgeStyle(COLORS.greenSoft,COLORS.green)}>🤝 {n}</span>)}</div></>}
              </div>
              ...
              <div className="hover-lift" style={cardStyle()}>
                ...
                <p style={{color:COLORS.muted,marginTop:10}}>Filtra il catalogo e scopri nomi coerenti con i tuoi gusti.</p>
