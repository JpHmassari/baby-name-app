...
const C = {
  bgTop: "#f5f3ff", bgBottom: "#f8fafc", card: "#ffffff", text: "#1f2330", muted: "#667085",
  border: "#ebe7f5", primary: "#7c3aed", primary2: "#a855f7", primary3: "#ec4899",
  primarySoft: "#f3e8ff", green: "#15803d", greenSoft: "#dcfce7", red: "#dc2626",
  redSoft: "#fee2e2", blue: "#2563eb", blueSoft: "#dbeafe", amber: "#d97706", amberSoft: "#fef3c7",
  slateSoft: "#f8fafc"
};

const pageStyle = { minHeight: "100vh", background: `linear-gradient(180deg, ${C.bgTop} 0%, ${C.bgBottom} 100%)`, padding: 20, fontFamily: "Inter, Arial, sans-serif", color: C.text };
const card = (extra={}) => ({ background: C.card, border: `1px solid ${C.border}`, borderRadius: 24, padding: 18, boxShadow: "0 10px 30px rgba(71,56,135,0.06)", ...extra });
const input = { padding: 14, width: "100%", boxSizing: "border-box", borderRadius: 14, border: `1px solid ${C.border}`, background: "#fff", color: C.text, outline: "none" };
const b = {
  primary:{ background:`linear-gradient(135deg, ${C.primary} 0%, ${C.primary2} 100%)`, color:"#fff", border:"none" },
  secondary:{ background:"#fff", color:C.text, border:`1px solid ${C.border}` },
  yes:{ background:C.greenSoft, color:C.green, border:"1px solid #bbf7d0" },
  no:{ background:C.redSoft, color:C.red, border:"1px solid #fecaca" },
  love:{ background:C.primarySoft, color:C.primary, border:"1px solid #e9d5ff" },
  warning:{ background:C.amberSoft, color:C.amber, border:"1px solid #fde68a" },
  activePill:{ background:C.primarySoft, color:C.primary, border:"1px solid #ddd6fe" },
};
const btn = (kind="primary") => ({ padding:"12px 16px", borderRadius:14, fontWeight:700, cursor:"pointer", transition:"all .2s ease", ...b[kind] });
const badge = (bg,color) => ({ display:"inline-flex", alignItems:"center", gap:6, padding:"7px 11px", borderRadius:999, background:bg, color, fontWeight:700, fontSize:13, whiteSpace:"nowrap" });
const voteBadge = (vote) => vote === "love" ? badge(C.primarySoft, C.primary) : vote === "yes" ? badge(C.greenSoft, C.green) : badge(C.redSoft, C.red);
const isPositiveVote = (v) => v === "yes" || v === "love";
...
export default function App() {
  ...
  useEffect(() => {
    if (profile?.id) {
      ...
      setVotes({}); setPartner(null); setPartnerVotes({}); setPriorityNames([]);
    }
  }, [profile]);
  ...
  const filteredNamePool = useMemo(() => {
    const base = deckFilter === "favorites" ? favoriteNames : deckFilter === "matches" ? matchedNames : namePool;
    if (deckFilter === "all" && priorityNames.length > 0) {
      const priorityUnvoted = priorityNames.filter((n) => base.includes(n) && !votes[n]);
      const rest = base.filter((n) => !priorityUnvoted.includes(n));
      return [...priorityUnvoted, ...rest];
    }
    return base;
  }, [deckFilter, favoriteNames, matchedNames, namePool, priorityNames, votes]);
  ...
  function refreshSmartSuggestions() {
    ...
    setPriorityNames(sorted.map((item) => item.name));
  ...
  async function createNewCouple() {
    ...
    try {
      ...
      setProfile(data); saveProfileLocally(data); setVotes({}); setPartner(null); setPartnerVotes({}); setPriorityNames([]); setMessage("Profilo creato! Il tuo codice coppia è: " + coupleCode); setJoinCode(""); setName("");
    } catch (err) { setMessage("Errore inatteso: " + err.message); }
  ...
  async function joinExistingCouple() {
    ...
    try {
      ...
      if (error) { setMessage("Errore Supabase: " + error.message); return; }
      setProfile(data); saveProfileLocally(data); setVotes({}); setPartner(null); setPartnerVotes({}); setPriorityNames([]); setMessage("Profilo collegato correttamente alla coppia " + normalizedCode); setName(""); setJoinCode("");
    } catch (err) { setMessage("Errore inatteso: " + err.message); }
    finally { setLoading(false); }
  }
  ...
  async function resetMyVotes() {
    ...
    try {
      ...
      setVotes({}); setDeckFilter("all"); setPriorityNames([]); setMessage("Tutti i tuoi voti sono stati azzerati");
    } catch (err) { setMessage("Errore inatteso: " + err.message); }
    finally { setVoteSaving(false); }
  }

  function logoutProfile() {
    clearSavedProfile(); setProfile(null); setVotes({}); setPartner(null); setPartnerVotes({}); setPriorityNames([]); setName(""); setJoinCode(""); setMessage("Profilo scollegato da questo dispositivo");
  }
  ...
  return (
    <div style={pageStyle}>
      ...
      {checkingSession ? (
        <div className="app-shell"><h1>Il Nome Perfetto</h1><p style={{color:C.muted}}>Caricamento profilo...</p></div>
      ) : !profile ? (
        <div className="app-shell" style={{maxWidth:780}}>
          <div className="hover-lift" style={card({padding:28,marginBottom:20,background:`linear-gradient(135deg, ${C.primarySoft} 0%, #ffffff 100%)`})}>
            <div style={badge(C.primarySoft, C.primary)}>✨ V10.1 Priority Suggestions</div>
            <h1 style={{fontSize:38,marginBottom:10}}>Il Nome Perfetto</h1>
            <p style={{color:C.muted,fontSize:16,lineHeight:1.6,marginBottom:0}}>I suggerimenti smart diventano i prossimi nomi da giudicare nel deck.</p>
          </div>
          ...
          <div className="content-grid" style={{marginTop:20}}>
            <div className="deck-panel hover-lift" style={card({padding:22,background:"linear-gradient(180deg, #ffffff 0%, #fcfcff 100%)"})}>
              <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:14}}>
                <div><h2 style={{margin:0}}>Deck con significato</h2><p style={{margin:"6px 0 0 0",color:C.muted,fontSize:14}}>Con meaning, origin, vibe e categorie visibili.</p></div>
              ...
              {votesLoading ? <p style={{color:C.muted}}>Caricamento voti...</p> : currentName && currentMeta ? (
                <div style={{position:"relative",borderRadius:30,padding:24,background:`linear-gradient(135deg, ${C.primary} 0%, ${C.primary2} 56%, ${C.primary3} 100%)`,color:"white",minHeight:390,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:"0 24px 50px rgba(124,58,237,0.24)",overflow:"hidden"}}>
                  ...
                  <div style={{position:"relative"}}>
                    ...
                    <div className="chip-wrap" style={{marginBottom:8}}>{[...currentMeta.styles.slice(0,3), currentMeta.vibe].map((label)=><span key={label} style={badge("rgba(255,255,255,0.18)","#fff")}>{label}</span>)}</div>
                  </div>
              ...
              ) : <div style={{textAlign:"center",padding:30,borderRadius:24,background:C.slateSoft,border:`1px solid ${C.border}`}}><h2 style={{marginBottom:8}}>Hai finito il deck 🎉</h2><p style={{color:C.muted,marginTop:0}}>Usa i suggerimenti smart o esplora nomi simili.</p></div>}
            </div>
            ...
            <div className="match-panel hover-lift" style={card({background: matchedNames.length ? "linear-gradient(180deg, #f0fdf4 0%, #ffffff 100%)" : "#ffffff"})}>
              <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><h2 style={{margin:0}}>Match di coppia</h2>{partner ? <div style={badge(C.primarySoft,C.primary)}>Partner: {partner.name}</div> : null}</div>
              {matchLoading ? <p style={{color:C.muted,marginTop:12}}>Caricamento match...</p> : !partner ? <div style={{marginTop:12}}><p style={{marginBottom:8}}><strong>Nessun partner collegato ancora.</strong></p><p style={{color:C.muted,marginTop:0}}>Condividi il tuo codice coppia: <strong>{profile.couple_code}</strong></p></div> : matchedNames.length === 0 ? <p style={{color:C.muted,marginTop:12}}>Per ora nessun match positivo.</p> : <><div style={{marginTop:12,marginBottom:14,padding:16,borderRadius:18,background:"rgba(34,197,94,0.08)",border:"1px solid #bbf7d0"}}><p style={{margin:0,color:C.muted,fontSize:14}}>Compatibilità percepita</p><h3 style={{marginTop:8,marginBottom:0,fontSize:34}}>{Math.min(100, 50 + matchedNames.length * 10)}%</h3></div><div className="chip-wrap">{matchedNames.slice(0,18).map((n)=><span key={n} style={badge(C.greenSoft,C.green)}>🤝 {n}</span>)}</div></>}
            </div>

            <div className="catalog-panel hover-lift" style={card()}>
              <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><h2 style={{margin:0}}>Catalogo smart (gratis)</h2><div style={badge(C.primarySoft,C.primary)}>{smartSuggestions.length} suggerimenti</div></div>
              ...
              <select value={exploreStyle} onChange={(e)=>setExploreStyle(e.target.value)} style={{...input,marginBottom:12}}><option value="all">Tutti gli stili</option>{allStyles.map((style)=><option key={style} value={style}>{style}</option>)}</select>
              <label style={{display:"block",marginBottom:6,color:C.muted,fontSize:14}}>Origine</label>
              <select value={exploreOrigin} onChange={(e)=>setExploreOrigin(e.target.value)} style={{...input,marginBottom:12}}><option value="all">Tutte le origini</option>{allOrigins.map((origin)=><option key={origin} value={origin}>{origin}</option>)}</select>
              <label style={{display:"block",marginBottom:6,color:C.muted,fontSize:14}}>Vibe</label>
              <select value={exploreVibe} onChange={(e)=>setExploreVibe(e.target.value)} style={{...input,marginBottom:12}}><option value="all">Tutti i vibe</option>{allVibes.map((vibe)=><option key={vibe} value={vibe}>{vibe}</option>)}</select>
              ...
              <button onClick={refreshSmartSuggestions} style={btn("primary")}>Aggiorna suggerimenti</button>
            </div>
            ...
            <div className="secondary-right secondary-grid">
              ...
              <div className="hover-lift" style={card()}>
                ...
                {recentVotes.length === 0 ? <p style={{color:C.muted}}>I tuoi ultimi voti appariranno qui.</p> : <div style={{display:"grid",gap:10,marginTop:12}}>{recentVotes.map((item)=><div key={item.babyName} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:12,borderRadius:16,background:C.slateSoft,border:`1px solid ${C.border}`}}><span style={{fontWeight:600}}>{item.babyName}</span><span style={voteBadge(item.vote)}>{item.vote}</span></div>)}</div>}
              </div>
            </div>
