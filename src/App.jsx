...
export default function App() {
  ...
  const favoriteNames = useMemo(
    () => randomNamePool.filter((n) => votes[n] === "yes" || votes[n] === "love"),
    [votes, randomNamePool]
  );

  const matchedNames = useMemo(
    () =>
      randomNamePool.filter(
        (n) => isPositiveVote(votes[n]) && isPositiveVote(partnerVotes[n])
      ),
    [votes, partnerVotes, randomNamePool]
  );
      ...
      {checkingSession ? (
        ...
        <div className="app-shell" style={{ maxWidth: 780 }}>
          <div className="hover-lift" style={cardStyle({ padding: 28, marginBottom: 20, background: `linear-gradient(135deg, ${COLORS.primarySoft} 0%, #ffffff 100%)` })}>
            <div style={badgeStyle(COLORS.primarySoft, COLORS.primary)}>✨ V10.2 stable</div>
            <h1 style={{ fontSize: 38, marginBottom: 10 }}>Il Nome Perfetto</h1>
            <p style={{ color: COLORS.muted, fontSize: 16, lineHeight: 1.6, marginBottom: 0 }}>Ordine random, suggerimenti in cima al deck e pulsante svuota priorità.</p>
          </div>
            ...
            <div className="deck-panel hover-lift" style={cardStyle({ padding: 22, background: "linear-gradient(180deg, #ffffff 0%, #fcfcff 100%)" })}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 14 }}>
                <div>
                  ...
                  <p style={{ margin: "6px 0 0 0", color: COLORS.muted, fontSize: 14 }}>Con meaning, origin, vibe e categorie visibili. Ordine sempre random.</p>
                </div>
